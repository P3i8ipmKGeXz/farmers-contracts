...
// File ReentrancyGuard.sol@0.8.0
/**
...
contract FarmersOnlyMasterChefL3 is Ownable, ReentrancyGuard {
    ...
    // Max withdraw fee: 20% (only used for King pools)
    uint256 public constant MAX_WITHDRAW_FEE = 2000;
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed onionPerSecond);
    ...
    function addPool(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint16 _withdrawFeeBP, uint256 _harvestInterval, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        ...
        require(_withdrawFeeBP <= MAX_WITHDRAW_FEE, "add: invalid withdraw fee basis points");
    ...
    function payOrLockupPendingOnion(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
